...
## [v0.9.0] — 2026-07-08 图片+文字合并发送

### 新增
- **附件暂存区**: composer 输入框上方新增 `.att-stage` 区域，选中的图片/文件以缩略图形式预览显示，支持逐个删除
- **多图+文字合发**: 一次消息可包含多张图片和一段文字，点击发送后统一上传+提交
- **stageFile 流程**: 选择文件后立即压缩、生成缩略图预览，而非直接上传发送
- **乐观更新含附件**: 发送时本地立即显示带附件的消息气泡，不用等服务器确认

### 变更
- **替换 sendOneFile/handleFiles**: 原有的"选文件立即单独发送"逻辑替换为暂存→合并发
- **doSend 重写**: 支持无文字纯图片发送、有文字无图片、以及任意组合
- **sw.js CACHE**: v25 → v26-combo
