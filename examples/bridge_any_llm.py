...
_CLASSIFY_PROMPT = (
    ...
    '{"level": 数字, "nsfw": 布尔}\n\n'
    "level 标准：\n"
    "1 = 轻松日常（打招呼、确认、单字、闲聊、简单问答、短句回应、随口聊天、"
    "日常分享、简短提问、语气词回应、简短感慨、状态汇报、随意吐槽，"
    "如「吃了吗」「今天好热」「我去洗澡了」「哈哈真的假的」「晚上吃的面条」"
    "「你在干嘛」「好无聊啊」「笑死了」「我想吃火锅」「今天加班好烦」"
    "「这个好好看」「啊真的吗」「我觉得还行」「随便聊聊天」）\n"
    "2 = 需要共情或深度回应（倾诉心事、明显情绪波动、认真提问需要深入思考、"
    "关系讨论、较长且认真的对话、分享重要经历或感受、需要安慰或建议）\n"
    "3 = 高情感/深度（重大情绪事件、争执冲突、深度亲密表达、长文认真讨论、"
    "人生重大决定）\n\n"
    "判断原则：日常聊天优先给 1，只有明确需要深度共情或认真回应时才升到 2。\n"
...
def classify_message(text: str, convo_tail: list) -> tuple:
    ...
    # 本地快速路径：短消息（≤12字）且无 NSFW → 直接 Haiku
    stripped = text.strip()
    if len(stripped) <= 12 and not nsfw_hint:
        log("tier", f"fast-path → haiku (len={len(stripped)}, text={stripped!r})")
        return (1, False)
